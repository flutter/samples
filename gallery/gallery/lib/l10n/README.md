...
For more details on what `flutter pub run grinder l10n` runs, you can read below under *How to Generate GalleryLocalizations with l10n scripts*. The current supported locales list is sorted alphabetically. This means that after running the script, you have to update
`gallery_localizations.dart` and move the `en_US` locale to the top of the list.
...
From `samples/gallery/gallery/`, run `dart ../l10n_cli/bin/main.dart`, which will generate
