 ...
 * @public
 */
export declare type NodeFor<ComplexSelector extends string> = TypeSelectorOfComplexSelector<ComplexSelector> extends infer TypeSelector ? TypeSelector extends keyof HTMLElementTagNameMap ? HTMLElementTagNameMap[TypeSelector] : TypeSelector extends keyof SVGElementTagNameMap ? SVGElementTagNameMap[TypeSelector] : Element : never;
...
declare type Split<Input extends string, Delemiter extends string, Acc extends string[] = []> = Input extends `${infer Prefix}${Delemiter}${infer Suffix}` ? Split<Suffix, Delemiter, [...Acc, Prefix]> : [...Acc, Input];
export {};
